...
static inline void clflush(char *data, int len, bool fence) {
  ...
  for (; ptr < data + len; ptr += CACHE_LINE_SIZE) {
    ...
    // #elif CLFLUSH_OPT
    //     asm volatile(".byte 0x66; clflush %0" : "+m"(*(volatile char
    //     *)(ptr)));
    // #elif CLWB
    asm volatile(".byte 0x66; xsaveopt %0" : "+m"(*(volatile char *)(ptr)));

    // update_counter.fetch_add(1);
    // #endif
  }
  if (fence)
    mfence();
...
void *leafnode::operator new(size_t size) {
  void *ptr = index_pmem_alloc(size);
...
leafvalue *masstree::make_leaf(const char *key, size_t key_len,
  ...
  // static thread_local void *thread_buffer = malloc(4096);
  ...
  if (value == 0) {
    ...
    aligned_alloc =
        index_pmem_alloc(sizeof(leafvalue) + len + sizeof(uint64_t));
  }
...
leafvalue *leafnode::smallest_leaf(size_t key_len, uint64_t value) {
  ...
  if (value == 0) {
    ...
    aligned_alloc = index_pmem_alloc(sizeof(leafvalue) + len);
  }
...
void masstree::put(const char *key, uint64_t value,
  ...
  if (kx_.p >= 0) {
    ...
    if (!IS_LV(l->value(kx_.p))) {
      ...
      // ii)  Atomically update value for the matching key
               ...
               memcmp(lv->fkey, (LV_PTR(l->value(kx_.p)))->fkey, lv->key_len) ==
                   0) {
...
void masstree::del(const char *key, ThreadInfo &threadEpocheInfo) {
  EpocheGuard epocheGuard(threadEpocheInfo);
  void *root = NULL;
  key_indexed_position kx_;
  uint32_t depth = 0;
...
leafnode *leafnode::search_for_leftsibling(void **root, uint64_t key,
  ...
  if (this->permutation.size() > LEAF_THRESHOLD) {
    ...
    if (depth > 0) {
      ...
      if (p->value(pkx_.p) == nr) {
        ...
        nl = search_for_leftsibling(&p->entry[pkx_.p].value,
      ...
      if (t->root() == nr) {
        ...
        nl = search_for_leftsibling(t->root_dp(),
                                    nr->highest ? nr->highest - 1 : nr->highest,
                                    nr->level_, nr);
...
void *masstree::get(const char *key, ThreadInfo &threadEpocheInfo) {
  EpocheGuard epocheGuard(threadEpocheInfo);
  void *root = NULL;
  key_indexed_position kx_;
  uint32_t depth = 0;
  ...
  if (needRestart)
    ...
    if (snapshot_v) {
      if (((leafvalue *)(snapshot_v))->key_len == lv->key_len &&
          memcmp(((leafvalue *)(snapshot_v))->fkey, lv->fkey, lv->key_len) ==
              0) {
