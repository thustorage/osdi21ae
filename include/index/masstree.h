...
public:
  ...
  void put(const char *key, uint64_t value, MASS::ThreadInfo &threadEpocheInfo);

  void put(const char *key, char *value, uint32_t value_size,
           MASS::ThreadInfo &threadEpocheInfo) {
    // char *ptr = (char *)index_pmem_alloc(sizeof(uint32_t) + value_size);
    // *(uint32_t *)ptr = value_size;
    // mempcpy(ptr + sizeof(uint32_t), value, value_size);
    // clflush(ptr, sizeof(uint32_t) + value_size, true);

    // put(key, (uint64_t)ptr, threadEpocheInfo);
  }

  void del(uint64_t key, MASS::ThreadInfo &threadEpocheInfo);

  void del(const char *key, MASS::ThreadInfo &threadEpocheInfo);

  void *get(uint64_t key, MASS::ThreadInfo &threadEpocheInfo);

  void *get(const char *key, MASS::ThreadInfo &threadEpocheInfo);

  bool get(const char *key, std::string &str, uint32_t value_size,
           MASS::ThreadInfo &threadEpocheInfo) {
    char *v = (char *)get(key, threadEpocheInfo);
    if (v == nullptr) {
      return false;
    }
  ...
  void split(void *left, void *root, uint32_t depth, leafvalue *lv,
             uint64_t key, void *right, uint32_t level, void *child,
             bool isOverWrite);

  int merge(void *left, void *root, uint32_t depth, leafvalue *lv, uint64_t key,
            uint32_t level, MASS::ThreadInfo &threadInfo);
  ...
  int scan(char *min, int num, uint64_t *buf,
           MASS::ThreadInfo &threadEpocheInfo);
};
...
public:
  ...
  static inline uint64_t make_sorted(int n) {
    uint64_t mask =
        (n == LEAF_WIDTH ? (uint64_t)0 : (uint64_t)16 << (n << 2)) - 1;
    return (make_empty() << (n << 2)) | ((uint64_t)FULL_VALUE & mask) | n;
  }

  /** @brief Return the permuter's size. */
  int size() const { return x_ & LEAF_WIDTH; }

  /** @brief Return the permuter's element @a i.
    @pre 0 <= i < width */
  int operator[](int i) const { return (x_ >> ((i << 2) + 4)) & LEAF_WIDTH; }

  int back() const { return (*this)[LEAF_WIDTH - 1]; }

  uint64_t value() const { return x_; }

  uint64_t value_from(int i) const { return x_ >> ((i + 1) << 2); }

  void set_size(int n) { x_ = (x_ & ~(uint64_t)LEAF_WIDTH) | n; }
  ...
  void remove(int i) {
    if (int(x_ & 15) == i + 1)
      ...
      uint64_t rot_mask = (((uint64_t)16 << rot_amount) - 1) << ((i + 1) << 2);
  ...
  void rotate(int i, int j) {
    uint64_t mask =
        (i == LEAF_WIDTH ? (uint64_t)0 : (uint64_t)16 << (i << 2)) - 1;
    // clear unused slots
    uint64_t x = x_ & (((uint64_t)16 << (LEAF_WIDTH << 2)) - 1);
  ...
  bool operator==(const permuter &x) const { return x_ == x.x_; }
  bool operator!=(const permuter &x) const { return !(*this == x); }

  int operator&(uint64_t mask) { return x_ & mask; }

  void operator>>=(uint64_t mask) { x_ = (x_ >> mask); }

  static inline int size(uint64_t p) { return p & 15; }
...
public:
  ...
  int split_into(leafnode *nr, int p, const uint64_t &key, void *value,
                 uint64_t &split_key);

  void split_into_inter(leafnode *nr, uint64_t &split_key);

  void *leaf_insert(masstree *t, void *root, uint32_t depth, leafvalue *lv,
                    uint64_t key, void *value, key_indexed_position &kx_);

  void *leaf_delete(masstree *t, void *root, uint32_t depth, leafvalue *lv,
                    key_indexed_position &kx_, MASS::ThreadInfo &threadInfo);

  void *inter_insert(masstree *t, void *root, uint32_t depth, leafvalue *lv,
                     uint64_t key, void *value, key_indexed_position &kx_,
                     leafnode *child, bool child_isOverWrite);

  int inter_delete(masstree *t, void *root, uint32_t depth, leafvalue *lv,
                   key_indexed_position &kx_, MASS::ThreadInfo &threadInfo);
  ...
  void make_new_layer(leafnode *p, key_indexed_position &kx_, leafvalue *olv,
                      leafvalue *nlv, uint32_t depth);

  leafnode *correct_layer_root(void *root, leafvalue *lv, uint32_t depth,
                               key_indexed_position &pkx_);

  void *entry_addr(int p);

  void check_for_recovery(masstree *t, leafnode *left, leafnode *right,
                          void *root, uint32_t depth, leafvalue *lv);

  void get_range(leafvalue *&lv, int num, int &count, uint64_t *buf,
                 leafnode *root, uint32_t depth);

  leafvalue *smallest_leaf(size_t key_len, uint64_t value);

  leafnode *search_for_leftsibling(void **root, uint64_t key, uint32_t level,
                                   leafnode *right);
};
